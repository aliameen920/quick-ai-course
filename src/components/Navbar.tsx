...
const Navbar = () => {
  ...
  return (
    <motion.nav
      ...
      className={`w-full py-5 px-4 sm:px-8 md:px-10 flex items-center justify-between sticky top-0 z-40 transition-all duration-300 ${
        scrolled ? "bg-[#132031] shadow-md border-b border-border/20" : "bg-[#132031]"
      }`}
    >
      ...
      >
        <img 
          src="/lovable-uploads/6a5a0a5a-3a32-4db2-a658-93393bb0c54f.png" 
          alt="MWPro Growth Logo" 
          className="h-40 md:h-26 object-contain" 
        />
        {!isMobile && (
            ...
            <SheetContent side="left" className="w-[280px] sm:w-[320px] overflow-y-auto bg-[#132031]">
              <div className="flex flex-col gap-6 mt-8">
                ...
                <div className="flex flex-col space-y-5">
                  {navItems.map(item => (
                    <a
                      key={item.name}
                      href={item.href}
                      className="text-xl font-medium text-white hover:text-mwpro-light-blue transition-colors"
                ...
                <div className="mt-6 pt-4 border-t border-white/20">
                  ...
                  <Button 
                    className="w-full mt-3 bg-mwpro-light-blue text-[#132031] hover:bg-mwpro-blue font-semibold text-base button-mwpro" 
                    size="lg" 
                    onClick={() => setMobileMenuOpen(false)}
                  >
                    Get Started
